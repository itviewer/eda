...
RoundedRectItemControlPoint::~RoundedRectItemControlPoint()
{

}

void RoundedRectItemControlPoint::mousePressEvent(QGraphicsSceneMouseEvent *mouseEvent)
{
    Q_UNUSED(mouseEvent);
...
void RoundedRectItemControlPoint::mouseMoveEvent(QGraphicsSceneMouseEvent *mouseEvent)
{
    if (opacity() != 0)
    {
...
void RoundedRectItemControlPoint::mouseReleaseEvent(QGraphicsSceneMouseEvent *mouseEvent)
{
    metaRect = boundingItem->rect();
    for (QGraphicsItem* controlPoint : boundingItem->childItems())
    {
    ...
    if(isMoved()){
        boundingItem->metadata["geometry"]["rect"] = metaRect;
