...
//    setViewportUpdateMode(QGraphicsView::MinimalViewportUpdate);
    ...
    schScene = new SchematicScene(this);
    setScene(schScene);

    connect(schEditor->actionButtonSnapToGrid,&QAction::triggered,this,[this](bool snap){
        snapToGrid = snap;
    });
    connect(schEditor,&SchematicEditor::sceneStateChanged,this,[this](const FSM &state){
...
SchematicView::~SchematicView()
{
    saveThread.quit();
    saveThread.wait();
    schScene->deleteLater();
}

void SchematicView::mousePressEvent(QMouseEvent *mouseEvent)
{
...
void SchematicView::mouseMoveEvent(QMouseEvent *mouseEvent)
{
//    qDebug() << dragMode() ;
    QGraphicsView::mouseMoveEvent(mouseEvent);
}

void SchematicView::mouseReleaseEvent(QMouseEvent *mouseEvent)
{
    // 需要先执行
...
void SchematicView::wheelEvent(QWheelEvent *mouseEvent)
{
...
void SchematicView::contextMenuEvent(QContextMenuEvent *menuEvent)
{
    // 确保右键菜单时鼠标下有内容
    if (!(itemsAt(menuEvent->pos()).isEmpty())){
        QGraphicsView::contextMenuEvent(menuEvent);
    }
}

/**
 * @brief 背景、前景、item都画在viewport上,在这里重写可以减少一次函数调用
 * @param painter
 * @param rect  delta 矩形
 */
...
void SchematicView::onDesignGridSizeChanged(int size)
{
    designGridSize = size;
}

// TODO 应该从底层解决否则会有冲突
QList<QGraphicsItem *> SchematicView::itemsAt(const QPoint &pos) const
{
    return items(QRect(pos - QPoint(3,3), QSize(6, 6)));
}

void SchematicView::resetGridSize()
{
...
void SchematicView::zoomIn()
{
    ...
    schEditor->setScaleStatus(QString("%1%").arg(currentScaleFactor * 100,0,'g',4));
...
void SchematicView::zoomHome()
{
    fitInView(schScene->pageBorderItem,Qt::KeepAspectRatio);
    currentScaleFactor = int(transform().m11() * 10000) / 10000.0;
    schEditor->setScaleStatus(QString("%1%").arg(currentScaleFactor * 100,0,'g',4));
