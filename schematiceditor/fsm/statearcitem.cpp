...
bool StateArcItem::mousePressEvent(QGraphicsSceneMouseEvent* mouseEvent)
{
    if (mouseEvent->button() == Qt::LeftButton)
    {
        ...
        if (!ellipseItem && !arcItem)
        {
            ...
            ellipseItem = new EllipseItem(schScene);
    ...
    if (mouseEvent->button() == Qt::LeftButton){
        if (arcItem)
        {
            ...
            if(circleArcRadius > 5){
                ...
                arcItem = new ArcItem(schScene);
                arcItem->setPos(ellipseItem->pos());
                schScene->addItem(arcItem);

                schScene->removeItem(ellipseItem);
                delete ellipseItem;
                ellipseItem = nullptr;
